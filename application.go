...
const casUrl = "https://secure.its.yale.edu/cas/login?service="

var cbUrl = url.QueryEscape("http://localhost:5000")
...
func HandleGetUrl(w http.ResponseWriter, r *http.Request) {
	...
	if ok == false {
		http.Error(w, "Failed to find url key.", 400)
	}

	skey := fmt.Sprintf("shorts:%s", base58)
	id, err := redis.Int64(credis.Do("GET", skey))
	if err == redis.ErrNil {
		http.Error(w, "Sorry. No url exists for this.", 404)
		return
	} else if err != nil {
		panic("Failed to GET id related to short.\n" + err.Error())
	}
	...
	skey = fmt.Sprintf("urls:%d", id)
	reply, err := redis.Values(credis.Do("HGETALL", skey))
	if err == redis.ErrNil || len(reply) == 0 {
		http.Error(w, "Sorry. No url exists for this.", 404)
		panic("urls:. hash not found for id " + string(id))
	} else if err != nil {
		panic("Failed to HGET.\n" + err.Error())
	}

	if err = redis.ScanStruct(reply, &p2); err != nil {
		panic("Failed to scanstruct urls:" + string(id))
	}

	fmt.Printf("%+v", p2)

	// http.Redirect(w, r, casUrl+cbUrl, 301)
	http.Redirect(w, r, p2.Url, 301)
}

func GET_UrlCallback(w http.ResponseWriter, r *http.Request) {
	return
}
...
func route() *mux.Router {
	...
	r.HandleFunc("/urls/{ckey}/callback", GET_UrlCallback).Methods("GET")
